...
| Wk | Topic (KOSEN68 source) | Kind | AIR-Sec hands-on core | Conventional source |
|----|-------|------|---|---|
| 1  | Intro + threat-modeling a crypto system | **Conceptual** ✅ *built* | Audit-the-AI: critique an AI answer for "why does textbook-secure crypto fail in real systems" | *(new framing, no direct KOSEN68 week)* |
| 2  | Hash functions | **Lab** ✅ *built* | Password-cracking a weakly-hashed user DB (unsalted MD5/SHA1) — deliberately distinct from `software-security` W3's ECB oracle | *(no Week2 Questions doc found — build fresh)* |
| 3  | MACs | **Lab** ✅ *built* | Hash-only auth-cookie forgery + HMAC length-extension | `Week3_MACs Questions.docx` (10 Q) |
| 4  | AES (block-cipher modes) | **Lab** ✅ *built* | CBC bit-flipping (malleability) tamper attack → why unauthenticated encryption fails; fixed with AES-GCM. Distinct from Wk6 (padding-oracle) and Wk10 (nonce-reuse audit) — together a why-authenticate → how → how-it-still-fails arc. (Pairs with source Q3/Q9 on unauthenticated encryption.) | `Week4_AES Questions.docx` (10 Q) |
| 5  | Key Exchanges | **Hybrid** ✅ *built* | MITM an unauthenticated DH exchange; small-subgroup confinement stays conceptual (EiPE) | `Week5_Key Exchanges Questions.docx` (10 Q) |
| 6  | Authenticated Encryption (AEAD) | **Lab** ✅ *built* | Padding-oracle attack on MAC-then-encrypt / encrypt-and-MAC, then rebuild with encrypt-then-MAC or AES-GCM — capstone of the symmetric block (Wk3 MAC + Wk4 AES combined correctly) | *(new topic, not in KOSEN68 — build fresh)* |
| **7** | **Reflection & Review (pre-midterm)** | — ✅ *built* | — | — |
| **8–9** | **Midterm** (Wk8 written · Wk9 practical) | — ✅ *built* | covers Wk 1–6 | — |
| 10 | Asymmetric & Hybrid Encryption | **Conceptual** ✅ *built* | Audit-the-AI: find the planted bug in an AI-generated hybrid-encryption implementation | `Week9 Questions — Asymmetric & Hybrid Encryption.docx` (5 Q) |
| 11 | Signatures & Zero-Knowledge Proofs | **Hybrid** ✅ *built* | Signature malleability demo (MtGox-style, txid mutation); Schnorr/ZKP stays conceptual (EiPE/Prompt Problem) | `Week10 Questions — Signature and ZKP.docx` (5 Q) |
| 12 | Secure Transport (TLS & Noise) | **Hybrid** ✅ *built* | Cert-validation-bypass MITM against a client that skips hostname checks; PKI trust-model comparison stays conceptual | `Week11 Questions.docx` (6 Q) |
| 13 | E2E Encryption | **Hybrid** ✅ *built* | Vuln/fixed pair: TLS-only chat server that *can* read messages vs. an E2E version that *can't* (server-log evidence) | `Week12 Questions.docx` (5 Q) |
| 14 | User Authentication | **Hybrid** ✅ *built* | Small demo: plaintext-password-over-TLS vs. a PAKE-style guarantee; FIDO2/SSO/TOTP comparison stays conceptual | *(no Questions doc found — build fresh)* |
| 15 | PQC | **Lab** ✅ *built* | Lamport one-time-signature key-reuse: sign twice, recover private-key bits | 60-Q final exam bank (6 Q on PQC) |
| 16 | Capstone studio (project WIP) | — ✅ *built (`project/README.md`)* | Term project: "CryptoVault" — teams compose KDF+AEAD+key-exchange+signatures+TLS correctly (see `project/README.md`; `EE2E Project.xlsx` was **not copied in**, rebuilt from scratch) | — |
| **17** | **Reflection & Review (pre-final)** | — ✅ *built* | — | — |
